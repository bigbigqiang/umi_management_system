...
class Recommend extends React.Component {
  ...
  getOperationPartList = () => {
    ...
    this.props.dispatch({
      type: 'recommend/getOperationPartList',
  ...
  delOperationPart = (partCode) => {
    this.props.dispatch({
      type: 'recommend/delOperationPart',
      payload: {
        partCode: partCode,
      }
    })
  ...
  addNewOne = () => {
    let { recommend: { partData } } = this.props;
    ...
    Modal.confirm({
      ...
      cancelText: '课程',
      onOk: () => {
        this.props.history.push(`/home/recommend/book/0?svipExist=${svipExist}`)
      },
      onCancel: () => {
        this.props.history.push(`/home/recommend/course/0?svipExist=${svipExist}`)
      },
    });
  }

  render() {
    const { loading, recommend: { partData, partTitle, goodsTypeList, goodsStateList } } = this.props;
...

    {
      ...
      render: (text, record, index) => {
        ...
        return (
          <ButtonGroup>
            <Button size='small' type="primary" icon="form" onClick={() => { this.props.history.push(`/home/recommend/${recommendType}/${record.partCode}`) }}></Button>
...
export default connect(({ recommend, loading }) => ({
  loading: loading.effects['recommend/getOperationPartList'],
  recommend
}))(Recommend);
