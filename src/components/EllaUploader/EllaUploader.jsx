  ...
  render() {
    ...
    const uploadProps = {
      ...
      beforeUpload(file) {
        ...
        typeof father.beforeUpload === 'function' && (result = father.beforeUpload(file))
        return result;
      },
      customRequest({
        action,
    ...
    let containerHeight =
      (this.props.height && this.props.height > 60 ? this.props.height : 60) + 'px';
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{
                display: `${this.state.preview_modal_show ? 'block' : 'none'}`,
                transition: 'all 0.2s',
              }}
              onClick={() => {
            ...
            >
              {/image/.test(fileType) ? (
                <img alt='' className={styles.previewImg} src={this.state.preview_link} />
              ) : (
                  ''
                )}
              {/video/.test(fileType) ? (
