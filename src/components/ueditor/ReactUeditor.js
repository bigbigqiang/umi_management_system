...
class ReactUeditor extends React.Component {
  ...
  componentDidMount() {
    ...
    // 解决切换路由再切回来时，上传图片 视频按钮消失，高度无效问题
    ...
    // ---------begin----------
    let _elements = window.document.getElementsByTagName('script')
    for (let i = 0; i < _elements.length; i++) {
      if (_elements[i].src.indexOf('/ueditor.config.js') !== -1 || _elements[i].src.indexOf('/ueditor.all.js') !== -1 || _elements[i].src.indexOf('/ueditor.all.min.js') !== -1) {
